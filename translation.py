...
def evaluateForBLEU(encoder, decoder):
    reference_text=[]
    generated_text=[]
    for pair in pairs:
        #print('>', pair[0])
        reference_text.append(pair[1])
        output_words, attentions = evaluate(encoder, decoder, pair[0], reverse)
        output_sentence = ''.join(output_words)
        generated_text.append(pair[1])
        #print('<', output_sentence)
        #print('')
...
evaluateForBLEU(encoder1, attn_decoder1)
...
evaluateAndShowAttention("he is not the man that he was when i first knew him")
